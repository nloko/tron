...
class Map
{
  public static readonly string[] MOVES = new string[] { "North", "East", "South", "West" }; 
  ...
  public static Mode DetermineMode()
  {
	//GameSimulator gs = new GameSimulator();
	//Point myPoint = new Point(gs.MyX(), gs.MyY());
//	Point opponentPoint = new Point(gs.OpponentX(), gs.OpponentY());

	//Room myRoom = new Room(gs);
	//float size = (float)myRoom.GetSize();

	//float ratio = (size / ((float)width * (float)height)) * 100;
	//Console.Error.WriteLine(size +" " + ratio);
	//if (ratio <= 25) {
	//	return Mode.Hide;
	//}
  ...
  public static bool IsNextToWall(int x, int y)
  {
